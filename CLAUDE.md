...
This system was built and used by [santifer](https://santifer.io) to evaluate job offers, generate tailored CVs. The archetypes, scoring logic, negotiation scripts, and proof point structure all reflect his specific career search.
...
### Periodic Maintenance (run manually every ~1-2 months)

| Script | What it does | When to run |
|--------|--------------|-------------|
| `node trim-scan-history.mjs` | Removes `scan-history.tsv` entries older than 90 days. Block G repost detection only looks back 90 days so older rows are dead weight. Use `--days N` to override. | When `data/scan-history.tsv` exceeds ~200 rows |
| `node archive-old-tracker.mjs` | Moves SKIP/Discarded/Rejected rows older than 60 days from `applications.md` to `data/applications-archive.md`. Keeps the active tracker lean. Use `--days N` to override. | When `applications.md` exceeds ~50 rows |
| Remove `.playwright-mcp/` artifacts | Session snapshots are gitignored but accumulate on disk. Delete the folder contents manually if disk space matters. | Occasionally |
