...
2. If file is empty or does not exist, try to get the JD from `{{URL}}` with WebFetch
...
Convert "builder" into a professional signal, not a "hobby maker". The framing changes, the truth remains the same.
...
3. **"If downleveled" plan**: accept if comp fair, 6-month review, clear criteria
...
| 8 | report | md link | `[647](reports/647-...)` | Report link |
