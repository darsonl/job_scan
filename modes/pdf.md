...
13. Get report number (`{num}`, zero-padded 3 digits) from context (auto-pipeline passes it as `REPORT_NUM`; if invoked as `pdf #073`, use it directly).
    Normalize company name to kebab-case lowercase (e.g. "Taipei European School" → "taipei-european-school") → `{company-slug}`.
14. Write HTML to `/tmp/{num}-{company-slug}-cv.html`
15. Run: `node generate-pdf.mjs /tmp/{num}-{company-slug}-cv.html output/{num}-{company-slug}-cv-{YYYY-MM-DD}.pdf --format={letter|a4}`
16. **Generate cover letter as Markdown (ALWAYS):** Write `output/{num}-{company-slug}-cl-{YYYY-MM-DD}.md` with the letter content. Format:
    - Heading with company, role, date, report number
    - 3-4 paragraphs: hook (why this company), hard skills match with metrics, AI/automation match or key differentiator, close with comp target + invitation
    - Signature with name, phone, email, LinkedIn, location
    - Language: same as the JD
    - Writing rules: no clichés, short sentences, action verbs, no passive voice (see `_writing-rules.md`)
17. If cover letter PDF is also requested: write HTML to `/tmp/{num}-{company-slug}-cl.html`, generate `output/{num}-{company-slug}-cl-{YYYY-MM-DD}.pdf` with the same format.
18. Report: CV PDF path, cover letter MD path, page count, % keyword coverage
