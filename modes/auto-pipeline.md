...
If input is a **URL** (not pasted JD text), follow this strategy to extract the content:

**Priority order:**

1. **Playwright (preferred):** Most job portals (Lever, Ashby, Greenhouse, Workday) are SPAs. Use `browser_navigate` + `browser_snapshot` to render and read the JD.
   **Pre-flight reset:** If the first `browser_navigate` fails with "Target page, context or browser has been closed" or another closed-browser error, call `browser_close` once to reset the MCP state, then retry. This error indicates an orphaned browser from a previous session.
...
Execute exactly as in `offer` mode (read `modes/oferta.md` for all blocks A-F + Block G Posting Legitimacy).
