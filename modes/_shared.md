...
| Playwright | Verify offers (browser_navigate + browser_snapshot). **NEVER 2+ agents with Playwright in parallel.** Call `browser_close` when done. If first `browser_navigate` fails with "Target page, context or browser has been closed", call `browser_close` once to reset, then retry. |
...
Rules for candidate-facing text are in `modes/_writing-rules.md`. Load that file for modes that generate CVs, cover letters, or LinkedIn messages (`pdf`, `auto-pipeline`, `apply`, `contacto`). Skip it for evaluation-only modes (`pipeline`, `oferta`, `ofertas`).
